                                       ...
                                       bool down, int clicks, bool invokePressEvents)
{
    ...
    if (mouse->mouse_touch_events) {
        if (mouseID != SDL_TOUCH_MOUSEID && mouseID != SDL_PEN_MOUSEID && button == SDL_BUTTON_LEFT) {
            ...
            if (window) {
                ...
                SDL_SendTouch(timestamp, SDL_MOUSE_TOUCHID, SDL_BUTTON_LEFT, window, type, normalized_x, normalized_y, 1.0f, invokePressEvents);
            }
        }
    }
    ...
    // Post the event, if desired
    if (SDL_EventEnabled(type) && invokePressEvents) {
        if ((!mouse->relative_mode || mouse->warp_emulation_active) && mouseID != SDL_TOUCH_MOUSEID && mouseID != SDL_PEN_MOUSEID) {
            // We're not in relative mode, so all mouse events are global mouse events
            mouseID = SDL_GLOBAL_MOUSE_ID;
        } else {
...
SDL_MouseButtonFlags SDL_GetMouseState(float *x, float *y)
{
    ...
    return SDL_GetMouseButtonState(mouse, SDL_GLOBAL_MOUSE_ID, invokeMouseButtonsEvents);
}
